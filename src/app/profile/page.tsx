...
import { AppleIcon, CalendarIcon, DumbbellIcon, TrashIcon } from "lucide-react";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
const ProfilePage = () => {
  ...
  return (
    <section className="relative z-10 pt-8 sm:pt-12 pb-24 sm:pb-32 flex-grow container mx-auto px-4">
      ...
      {allPlans && allPlans.length > 0 ? (
        <div className="space-y-6 sm:space-y-8">
              ...
              {allPlans.map((plan) => (
                <div key={plan._id} className="relative group">
                  <Button
                    ...
                    className={`text-sm sm:text-base text-foreground border hover:text-white transition-all ${
                      selectedPlanId === plan._id
                        ? "bg-primary/20 text-primary border-primary"
                        : "bg-transparent border-border hover:border-primary/50"
                    } ${plan.isActive ? "border-green-500/30" : ""}`}
                  >
                    {plan.name}
                    {plan.isActive && (
          ...
          {currentPlan && (
            <div className="relative backdrop-blur-sm border border-border rounded-lg p-4 sm:p-6">
              <CornerElements />
              ...
              <Tabs defaultValue="workout" className="w-full">
                <TabsList className="mb-4 sm:mb-6 w-full grid grid-cols-2 bg-cyber-terminal-bg border">
                  <TabsTrigger
                    value="workout"
                    className="data-[state=active]:bg-primary/20 data-[state=active]:text-primary text-xs sm:text-sm"
                  >
                    <DumbbellIcon className="mr-2 size-3 sm:size-4" />
                    Workout
                  </TabsTrigger>
                  <TabsTrigger
                    value="diet"
                    className="data-[state=active]:bg-primary/20 data-[state=active]:text-primary text-xs sm:text-sm"
                  >
                    <AppleIcon className="mr-2 h-3 w-3 sm:h-4 sm:w-4" />
                    Diet
                  </TabsTrigger>
                </TabsList>
                    ...
                    <Accordion type="multiple" className="space-y-3 sm:space-y-4">
                      {currentPlan.workoutPlan.exercises.map(
                        (exerciseDay, index) => (
                          ...
                          >
                            <AccordionTrigger className="px-3 sm:px-4 py-2 sm:py-3 hover:no-underline hover:bg-primary/10 font-mono">
                            ...
                            <AccordionContent className="pb-2 sm:pb-4 px-3 sm:px-4">
                              <div className="space-y-2 sm:space-y-3 mt-1 sm:mt-2">
                                {exerciseDay.routines.map(
                                  (routine, routineIndex) => (
                                    <div
                                      key={routineIndex}
                                      className="border border-border rounded p-2 sm:p-3 bg-background/50"
                                    >
                                      <div className="flex flex-col sm:flex-row justify-between items-start gap-2 mb-1 sm:mb-2">
                                        <h4 className="font-medium text-foreground text-sm sm:text-base">
                                          {routine.name}
                                        </h4>
                                        <div className="flex items-center gap-2">
                                          <div className="px-2 py-1 rounded bg-primary/20 text-primary text-2xs sm:text-xs font-mono">
                                            {routine.sets} SETS
                                          </div>
                                          <div className="px-2 py-1 rounded bg-secondary/20 text-secondary text-2xs sm:text-xs font-mono">
                                            {routine.reps} REPS
                                          </div>
                                        </div>
                                      </div>
                      ...
                      {currentPlan.dietPlan.meals.map((meal, index) => (
                        ...
                        >
                          <div className="flex items-center gap-2 mb-2 sm:mb-3">
                            ...
                            <h4 className="font-mono text-xs sm:text-sm text-primary">
                              {meal.name}
                            </h4>
                          </div>
                          <ul className="space-y-1 sm:space-y-2">
                            {meal.foods.map((food, foodIndex) => (
                              ...
                              >
                                <span className="text-2xs sm:text-xs text-primary font-mono">
