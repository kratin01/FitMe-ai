...
const GenerateProgram = () => {
  ...
  const messageContainerRef = useRef<HTMLDivElement>(null); //we are using typescript so we need to define the type of the ref
  ...
  useEffect(() => {
    //this is how we can done a auto scroll in a div
  ...
  //Navigate the user to the profile page when the call is ended
  useEffect(() => {
    if (callEnded) {
  ...
  //Setting up the event listeners for the vapi object
  useEffect(() => {
    ...
    //add event listeners to the vapi object
    vapi
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          ...
          <Card className="bg-card/90 backdrop-blur-sm border border-border overflow-hidden relative">
            <div className="aspect-video flex flex-col items-center justify-center p-6 relative">
              ...
              <h2 className="text-xl font-bold text-foreground">FitMe AI</h2>
              ...
              {messages.map((msg, index) => (
                <div key={index} className="message-item animate-fadeIn">
                  <div className="font-semibold text-xs text-muted-foreground mb-1">
                    {msg.role === "assistant" ? "FitMe AI" : "You"}:
                  </div>
